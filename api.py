"""ASH08 API — paper book with P&L + LTP + Upstox status. Start: python api.py"""
...
def load_mods():
    ...
    try:
        ...
        m["Metrics"] = StockMetrics; m["run_scan"] = run_scan
    except Exception as e:
    ...
    try:
        ...
        m["fetch_nse"] = fetch_nse_equity_instruments; m["fetch_quotes"] = fetch_quotes; m["profile"] = user_profile
    except Exception as e:
        LOG.error("upstox: %s", e)
    try:
        from ash08.universe import InstrumentRow, UniverseManager
        m["Row"] = InstrumentRow; m["Uni"] = UniverseManager
    except Exception as e:
...
def get_engine():
    ...
    if "PaperEngine" not in MODS: return None
    eng = MODS["PaperEngine"](data_dir=str(DATA_DIR))
    p = DATA_DIR / "paper_state.json"
    if p.exists():
        try:
            st = json.loads(p.read_text())
            eng.orders = st.get("orders") or []; eng.positions = st.get("positions") or []
...
def upstox_status():
    ...
    if not tok: return info
    if "profile" not in MODS:
        info["detail"] = "token set; upstox module missing"; return info
    try:
        prof = MODS["profile"]()
        info["connected"] = True; info["detail"] = "profile ok"
        data = prof.get("data") if isinstance(prof, dict) else None
        if isinstance(data, dict):
            info["user"] = data.get("user_name") or data.get("email") or "ok"
    except Exception as e:
        info["detail"] = f"token set but API failed: {e}"
    return info

def quotes_for_symbols(symbols):
    if "fetch_quotes" not in MODS: return {}
    if not (os.environ.get("UPSTOX_ACCESS_TOKEN") or "").strip(): return {}
    keys = [f"NSE_EQ|{s}" for s in symbols if s]
    if not keys: return {}
    try:
        raw = MODS["fetch_quotes"](keys)
    except Exception as e:
        LOG.warning("quotes: %s", e); return {}
    out = {}
    for k, v in (raw or {}).items():
        if not isinstance(v, dict): continue
        sym = k.split("|")[-1] if "|" in k else k
        lp = v.get("last_price") or v.get("lastPrice")
        if lp is None and isinstance(v.get("ohlc"), dict): lp = v["ohlc"].get("close")
        if lp is not None:
            try: out[sym.upper()] = float(lp)
            except Exception: pass
    return out

def auto_buy_from_scan(scan_dict):
    eng = get_engine()
    if not eng or not hasattr(eng, "auto_buy_selects"): return None
    ...
    for r in selects:
        ...
        if r.get("ltp") and sym not in price_map:
            try: price_map[sym] = float(r["ltp"])
            except Exception: pass
    try:
        result = eng.auto_buy_selects(selects, price_map=price_map)
        LOG.info("auto_select buys=%s open=%s", result.get("bought"), result.get("open_count"))
        return result
    except Exception as e:
        LOG.exception("auto_buy"); return {"error": str(e)}
...
def seed_demo_local():
    ...
    for i, s in enumerate(symbols[:400]):
        mom = 0.14 - (i % 9) * 0.015; qual = 78 - (i % 11) * 2
        ltp = REF_LTP.get(s, 100.0 + (i % 50) * 3)
        metrics.append(Metrics(symbol=s, adv20=350000, turnover_cr_5d=12, stale_days=0,
                               mom_6m=mom, quality_score=qual, ltp=ltp))
    snap = MODS["run_scan"](metrics, universe_bucket="core")
    scan_dict = snap.to_dict(); store.save_scan(scan_dict)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/paper/buy": return self.api_paper_buy(body)
    ...
    def do_GET(self):
        ...
        if path == "/api/scan/run": return self.json(200, seed_demo_local())
        if path == "/api/demo/run": return self.json(200, seed_demo_local())
        if path == "/api/paper/book": return self.api_paper_book()
        ...
        if path == "/api/paper/auto":
            ...
            return self.json(200, {"ok": True, "auto_paper": auto, "upstox": upstox_status()})
    ...
    def api_paper_book(self):
        ...
        if live and hasattr(eng, "mark_to_market"): eng.mark_to_market(live)
    ...
    def api_paper_buy(self, body):
        ...
        try:
            ...
            if hasattr(eng, "mark_to_market"): eng.mark_to_market({symbol: price})
        except Exception as e:
...
def main():
    DESK.mkdir(parents=True, exist_ok=True); DATA_DIR.mkdir(parents=True, exist_ok=True)
    try: seed_demo_local()
    except Exception as e: LOG.warning("seed: %s", e)
    get_engine()
    LOG.info("ASH08 on 0.0.0.0:%s paper=%s core=%s upstox=%s", PORT, "PaperEngine" in MODS, CORE_COUNT, upstox_status().get("detail"))
